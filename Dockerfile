# ─────────────────────────────────────────────────────────────────────────────
# Portuguese Competition (AdC) MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t portuguese-competition-mcp .
# Run:    docker run --rm -p 3000:3000 portuguese-competition-mcp
#
# The image bakes /app/data/adc.db at build time. Override ADC_DB_PATH
...
ENV ADC_DB_PATH=/app/data/adc.db
...
# Bake the pre-built database into the image so /app/data/adc.db resolves
...
# We then COPY it into the image at /app/data/adc.db (ADC_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/adc.db
